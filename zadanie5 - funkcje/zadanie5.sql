...
-- dodanie zmiennych Aktywny dla Lotów oraz Samolotów
ALTER TABLE Loty ADD Aktywny BOOL DEFAULT 1;
ALTER TABLE Samoloty ADD Aktywny BOOL DEFAULT 1;



-- dodanie kompletu powiązanych ze sobą rekordów, dla całkowice pustej bazy

INSERT INTO Trasy (IdTrasy,LotniskoStart, LotniskoCel, OdlegloscKm)
VALUES 
(1,'Warszawa', 'Londyn', 1500),
(2,'Kraków', 'Berlin', 800),
(3,'Gdańsk', 'Paryż', 1200);


INSERT INTO Samoloty (IdSamolotu, Model, Rodzaj, Zasieg, Spalanie, LiczbaMiejsc_Ekonomiczna, LiczbaMiejsc_Biznes, LiczbaMiejsc_Pierwsza, Aktywny) 
VALUES 
(1,'Boeing 737', 'Jet', 3000, 120, 100, 20, 10, 1),
(2,'Airbus A320', 'Jet', 3500, 150, 120, 25, 15, 1),
(3,'Embraer 190', 'Turboprop', 1500, 80, 60, 10, 5, 1);


INSERT INTO Loty (IdSamolotu, IdTrasy, DataStartu, GodzinaStartu, GodzinaMety, Aktywny) 
VALUES 
(1, 1, '2024-12-10', '10:00:00', '14:00:00', 1),
(2, 2, '2024-12-12', '15:00:00', '18:00:00', 1),
(3, 3, '2024-12-15', '09:00:00', '12:00:00', 1);


INSERT INTO Bilety (IdLotu, Imie, Nazwisko, NumerTelefonu, NumerPaszportu, Rzad, Miejsce, Klasa) VALUES 
(4, 'Jan', 'Kowalski', '123456789', 'AB1234567', 1, 'A', 'Ekonomiczna'),
(2, 'Anna', 'Nowak', '987654321', 'BC2345678', 1, 'B', 'Biznes'),
(3, 'Piotr', 'Wiśniewski', '123123123', 'CD3456789', 2, 'C', 'Pierwsza');



-- funkcja wypisująca nazwisko plus klase 
DELIMITER // 

CREATE FUNCTION wyswietl(IdBiletu INT)
RETURNS VARCHAR(200)
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE nazwisko_wynik VARCHAR(50);
    DECLARE klasa_wynik VARCHAR(50);
    DECLARE wynik VARCHAR(200);

    -- Pobranie danych tylko dla aktywnych lotów i samolotów
    SELECT Bilety.Nazwisko, Bilety.Klasa
    INTO nazwisko_wynik, klasa_wynik
    FROM Bilety
    JOIN Loty ON Bilety.IdLotu = Loty.IdLotu
    JOIN Samoloty ON Loty.IdSamolotu = Samoloty.IdSamolotu
    WHERE Bilety.IdBiletu = IdBiletu
    AND Loty.Aktywny = 1
    AND Samoloty.Aktywny = 1;


    IF nazwisko_wynik IS NOT NULL THEN
        SET wynik = CONCAT('Nazwisko: ', nazwisko_wynik, ', Klasa: ', klasa_wynik);
    END IF;
...
-- wywołanie funkcji 1)
SELECT IdBiletu, wyswietl(IdBiletu) AS InformacjePasażera
FROM Bilety;


-- 2) usunąć jeden z samolotów oraz jeden z lotów 
--  zamiana wartoscie zmiennej Aktywny na 0

UPDATE Samoloty SET Aktywny = 0 WHERE IdSamolotu = 3;
UPDATE Loty SET Aktywny = 0 WHERE IdLotu = 3;


-- ponowne wywołanie funkcji 1
SELECT IdBiletu, wyswietl(IdBiletu) AS InformacjePasażera
FROM Bilety;
